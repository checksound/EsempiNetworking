...
/**
 * This class is part of a demonstration of distributed computing. It is to be
 * used with CLMandelbrotTask.java on the master computer. Before starting
 * CLMandelbrotMaster on the master computer, CLMandelbrotWorker should be
 * started as a command-line program on each worker computer involved in the
 * distributed computation. See CLMandelbrotWorker.java for more information.
 * (It is possible to run several copies of CLMandelbrotWorker on the same
 * computer, as long as each copy uses a different port number. It is even
 * possible to run the master and one or more copies of the worker on the same
 * computer. In that case, the host name for the workers would be localhost.)
 * 
 * If CLMandelbrotMaster is run with no command line arguments, it will run
 * without using the network at all. The complete computation will be done
 * locally (using just one processor). You can do this to test how long the
 * computation takes when distributed computing is not used.
 * 
 ...
 * command line argument gives the host name or IP address of a computer on
 * which a worker is running. If that worker is not listening on the default
 * port, then the listening port number must also be included. The form for
 * specifying a computer with a port number is, for example, math.hws.edu:1501
 * or 127.0.0.1:18881, that is, the computer name or IP, followed by a colon,
 * followed by the port number, with NO SPACES.
 * 
 * When CLMandelbrotMaster runs, it creates a list of tasks (of type
 * CLMandelbrotTask) that have to be performed, and it creates a thread for
 * communicating with each copy of CLMandelbrotWorker. Each thread sends a
 * sequence of tasks to the connected worker, which does the actual work
 * involved in performing the task and sends back the results.
 * 
 * Although this program is meant as a demonstration of distributed computing,
 * it does compute an interesting picture. If you want to see that picture,
 * uncomment the call to saveImage() at the end of the main() routine. The
 * program computes the same picture every time it is run.
 * 
 * Note that data sent over the network is encoded as text. The first word on a
 * line of text identifies the type of data.
 */
public class CLMandelbrotMaster {
	...
	/**
	 * The first and only word on a message representing a shutdown command. (This
	 * feature is currently unused.)
	 */
	private static final String SHUT_DOWN_COMMAND = "shutdown";

	/**
	 * The first word on a message representing a CLMandelbrotTask. This is followed
	 * by the incoming data (id, maxIterations, y, xmin, dx, and count) for the
	 * task. Items on the line are separated by spaces.
	 */
	private static final String TASK_COMMAND = "task";

	/**
	 * The first word on a message representing the results from a CLMandelbrotTask.
	 * This is followed by the task id, the number of items in the results, and then
	 * the results. Items on the line are separated by spaces.
	 */
	private static final String RESULT_COMMAND = "result";

	/**
	 * The list of tasks that must be performed to complete the computation. This
	 * list is created by createJob(). The tasks are sent out to workers to be
	 * performed. Each task represents the computation of one row of an image of
	 * part of the Mandelbrot set.
	 */
	private static ConcurrentLinkedQueue<CLMandelbrotTask> tasks;

	/**
	 * The number of tasks that have been completed. At the end of the computation,
	 * this should be equal to the number of rows in the image that is being
	 * computed.
	 */
	...
	/**
	 * Maximum number of iterations for the Mandelbrot computation; set by
	 * computeJob(). For the purposes of this computation, it is not necessary to
	 * understand this.
	 */
	private static int maxIterations;

	/**
	 * All the data for the image, collected from the results of all the tasks.
	 * mandelbrotData[c][r] is the data for row r, column c in the image.
	 */
	private static int[][] mandelbrotData;

	/**
	 * The main program opens network connections to CLMandelbrotWorker programs and
	 * uses them to compute a visualization of a small piece of the famous
	 ...
	 *             the computers on which the CLMandelbrotWorker program is running,
	 *             including port numbers for the worker programs that are not
	 *             listening on the default port. (See the main comment on this
	...
	/**
	 * Creates the data needed for the computation and the list of tasks that will
	 * perform parts of the computation. For the purposes of this computation, it is
	 * not necessary to understand the computation.
	 */
	...
	/**
	 * We allow for the possibility that a thread might fail while it is performing
	 * a task. When that happens, the thread drops the task back into the list of
	 * tasks so that it can be assigned to another worker. That way, it is likely
	 * that all the tasks will be completed even if some of the worker threads fail.
	 * (However, this is not completely foolproof -- if the failure occurs after
	 * other worker threads have already terminated, there won't be any threads left
	 * to execute the task.)
	 */
	...
	/**
	 * Add the data from a finished task to the array where the complete set of data
	 * is collected. Also increments tasksCompleted. This method is synchronized
	 * because of the race condition involved in incrementing tasksCompleted.
	 */
	...
	/**
	 * Encode a CLMandelbrotTask into a String that can be sent as a message over
	 * the network to one of the worker programs.
	 */
	 ...
	 * 
	 * @param data the message that contains the results. It is already known that
	 *             the first word of the message is RESULT_COMMAND.
	 * @param task the task for which results are expected. The task id in the
	 *             results message must match the id of this task. The results are
	 *             stored as the value of task.results.
	...
	/**
	 * This class represents one worker thread. The job of a worker thread is to
	 * send out tasks to a CLMandelbrotWorker program over a network connection, and
	 * to get back the results computed by that program.
	 */
	private static class WorkerConnection extends Thread {
		...
		/**
		 * The constructor just sets the values of the instance variables id, host, and
		 * port and starts the thread.
		 */
		...
		/**
		 * The run() method of the thread opens a connection to the host and port
		 * specified in the constructor, then sends tasks to the CLMandelbrotWorker
		 * program on the other side of that connection. If the thread terminates
		 * normally, it outputs the number of tasks that it processed. If it terminates
		 * with an error, it outputs an error message.
		 */
		public void run() {
			...
			try {
				...
				System.out.println("Thread " + id + " could not open connection to " + host + ":" + port);
	...
	/**
	 * Although this program is meant as a demonstration of distributed computing,
	 * it does compute something interesting. This method will save the image that
	 * was computed by the program to a file. It first asks the user whether the
	 * user wants to save the image. If the answer is yes, a file dialog box is put
	 * up where the user can specify the file in which the image is to be saved.
	 * NOTE that this method will not be called unless you uncomment the call to
	 * this method at the end of the main program.
	 */
